...
if (isset($_GET['getScores'])) {
	...
	if (isset($_GET['game_id'])) {
		$query = "SELECT score, submitted_by FROM high_scores WHERE game_id = '".$_GET['game_id']."' ORDER BY score";
		$result = $handle->query($query) or die ("Could not retrieve scores for that game :: ".mysql_error());
		$xml = '<?xml version="1.0" encoding="utf-8"?>';
		$xml .= '<scores gameid="'.$_GET['gameid'].'">';
...
if (isset($_GET['saveScore'])) {
	if (isset($_GET['score']) && isset($_GET['game_id']) && isset($_GET['user_id'])) {
		$score = $validate->var_prep($_GET['score']);
		$submitted_by = $validate->var_prep($_GET['user_id']);
		$game_id = $validate->var_prep($_GET['game_id']);
		$query = "INSERT INTO high_scores (score, game_id, submitted_at, user_id, submitted_by_ip) VALUES 
				($score, $game_id, NOW(), $submitted_by, '".getRealIpAddr()."')";
		...
		echo "Could not save score -- was missing some information.";
	}
	die();
}
